...
export function AvoltaVoiceShop({ catalog, flightDay }) {
  ...
  const buildTools = useCallback((tool, z) => {
    ...
    const confirmTool = tool({ name: "confirm_demo_order", description: "Confirm the exact reviewed order only in the response turn immediately after the traveler gives an unambiguous spoken approval to ‘Do you confirm the order?’. Call silently and announce success only when the result is ok. No payment, stock hold, store message or dispatch occurs.", parameters: z.object({ review_fingerprint: z.string().min(1) }), execute: async ({ review_fingerprint }) => { if (reservationRef.current?.fingerprint === review_fingerprint) return safe(createReservation(review_fingerprint)); const approvalState = voiceReviewApprovalRef.current; const confirmationReply = await waitForConfirmationReply(approvalState); const sessionIsCurrent = isApprovalSessionCurrent({ approvalState, currentApprovalState: voiceReviewApprovalRef.current, currentGeneration: voiceStartSequenceRef.current, currentSession: sessionRef.current }); const approval = sessionIsCurrent ? validateSpokenOrderApproval({ review: reviewRef.current, expectedFingerprint: review_fingerprint, reviewUserItemId: approvalState.reviewUserItemId, confirmationReply }) : { ok: false, reason: "approval_cancelled", error: "The order confirmation was cancelled. Review the order again before confirming." }; if (!approval.ok) voiceReviewApprovalRef.current = null; return safe(approval.ok ? createReservation(review_fingerprint) : approval); } });
...
- If confirm_demo_order returns an error, do not announce success. Explain the one recoverable next step briefly. If it succeeds, announce the confirmed reference and fulfillment destination once.
...
Initial interface state: ${safe(stateSnapshot())}` });
  ...
  return (
    <main className={styles.page} data-audio-track={audioEvidence.trackReceived ? "received" : "none"} data-audio-model={audioEvidence.modelAudioStarted ? "started" : "waiting"} data-audio-bytes={audioEvidence.bytesReceived} data-audio-energy={audioEvidence.totalAudioEnergy} data-audio-playback={playbackState} data-voice-tool-starts={voiceEventEvidence.toolStarts} data-voice-tool-ends={voiceEventEvidence.toolEnds} data-voice-spoken-turns={voiceEventEvidence.spokenTurns} data-voice-last-tool={voiceEventEvidence.lastTool}>
