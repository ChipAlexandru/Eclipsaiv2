...
- **Quantities are untouched:** €40–60K, €190K, 860,000, 92%, 1 in 4, 86%,
  16 months, 2 to 20 locations, 130 fewer, and all replay data. Only decimal
  and thousands separators follow local convention.
...
| the daily decisions that decide profit | die täglichen Entscheidungen, die über den Gewinn bestimmen | Long. Confirm it fits the offer headline without feeling bureaucratic. |
| Start free | Kostenlos starten | Standard. Confirm it does not imply a free tier forever. |
...
| the daily decisions that decide profit | les décisions quotidiennes qui déterminent le profit | Confirm `profit` vs `rentabilité` for an owner audience. |
| Start free | Commencer gratuitement | Longest CTA of the five locales; check button width on mobile. |
...
| the daily decisions that decide profit | le decisioni quotidiane che determinano il profitto | Confirm `profitto` vs `redditività`. |
| Start free | Inizia gratis | Informal imperative while body copy uses `voi`. Deliberate for a button; confirm the mix is acceptable. |
...
| the daily decisions that decide profit | deciziile zilnice care hotărăsc profitul | Deliberately avoids the awkward `care decid`. Confirm `hotărăsc` is not too strong. |
| Start free | Începeți gratuit | Polite plural, matching page register. |
