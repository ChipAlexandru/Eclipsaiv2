export const INITIAL_DEMO_HANDLES = [
  "chnebelbrot", "der-alte-fritz", "sesamleinsamenbrot", "chorblibrot",
  "franzosisch-patisserie-zitrone", "erdbeer-torte", "schwarzwaldertorte",
  "tiramisu", "mini-patisserie-assortiert", "tomaten-mozarella-salat",
  "gemischter-salat", "hamburger", "sandwiches", "hauskonfekt",
  "chocolierte-mandeln", "glace-stengel",
];
...
export function changeBasket(basket, productId, quantity, mode, validProductIds, optionId = "standard") {
  if (!validProductIds.has(productId)) throw new Error("Unknown product ID.");
  const lineKey = lineKeyFor(productId, optionId);
  const amount = Math.max(0, Math.min(24, Math.trunc(Number(quantity) || 0)));
  const current = basket[lineKey] || 0;
  let nextQuantity;
  ...
  const next = { ...basket };
  if (nextQuantity === 0) delete next[lineKey];
  else next[lineKey] = nextQuantity;
  return next;
}
...
export function basketSummary(basket, productsById) {
  const items = Object.entries(basket)
    .map(([lineKey, quantity]) => {
      ...
      return product ? (optionId === "standard"
        ? { productId, name: product.name, quantity }
        : { lineKey, productId, optionId, name: product.name, quantity }) : null;
    })
    .filter(Boolean);
...
export function shoppingStateSnapshot({ visibleIds, selectedId, basket }, productsById) {
  return {
    ...
    availabilityBasis: "Branch availability is deterministic sample data, not a live inventory feed. Confirm current availability with Bäckerei Hausammann.",
    priceBasis: "CHF amounts are synthetic demo prices, not current verified Hausammann prices.",
  };
}

export function compactProduct(product) {
  if (!product) return null;
  return { id: product.id, name: product.name, productType: product.productType };
}
...
import { lineKeyFor, parseLineKey } from "./commerce.mjs";
