      ...
      <section className={styles.productSurface} aria-label="Zürich Duty Free products">
        <div className={styles.contextRail} data-has-order={Boolean(order)}>
          ...
          {order && orderState && <article className={styles.orderCard}>
            ...
            <div className={styles.progressTrack} aria-label={`Order progress: ${orderState.state}`}>{orderState.path.map((step, index) => <span key={step} data-complete={index <= orderState.stateIndex} data-current={index === orderState.stateIndex}><i /><em>{step}</em></span>)}</div>
