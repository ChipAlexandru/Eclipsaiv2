...
function searchAliases(product) {
  ...
  const aliases = [];
  if (/brot|bread|baguette|parisette|weggli|gipfel/.test(value)) aliases.push("bread loaf bakery staple");
  if (/croissant|gipfel|weggli|bretzel|brioche/.test(value)) {
    aliases.push("viennoiserie breakfast pastry sweet");
  }
  if (/tart|torte|tortchen|cake|cremeschnitte|mousse|roulade|charlotte|patisserie/.test(value)) {
    aliases.push("patisserie pastry dessert sweet cake");
  }
  if (/sandwich|salat|quiche|canape|laugen|traiteur|parisette/.test(value)) {
    aliases.push("lunch savoury savory");
  }
  if (/gutschein|voucher/.test(value)) aliases.push("gift voucher");
  if (/praline|truff|schokolade|chocolat|chakra|tartufi/.test(value)) aliases.push("chocolate confectionery pralines gift");
  return aliases.join(" ");
}

export function changeBasket(basket, productId, quantity, mode, validProductIds) {
  if (!validProductIds.has(parseBasketKey(productId).productId)) throw new Error("Unknown product ID.");
...
export function basketSummary(basket, productsById) {
  const items = Object.entries(basket)
    .map(([basketKey, quantity]) => {
      const { productId, options } = parseBasketKey(basketKey);
      const product = productsById.get(productId);
      if (!product) return null;
      const unitPriceChf = Number((product.priceChf + optionDelta(options)).toFixed(2));
      return {
        basketKey,
        productId,
        name: product.name,
        ...
        quantity,
        unitPriceChf,
        lineTotalChf: Number((unitPriceChf * quantity).toFixed(2)),
      };
    })
    .filter(Boolean);
  return {
...
export function shoppingStateSnapshot({ visibleIds, selectedId, basket }, productsById) {
  return {
    ...
    stockBasis: "Availability is illustrative and not live store inventory; confirm with Honold.",
  };
}
...
import { optionDelta, parseBasketKey } from "./experience.mjs";
